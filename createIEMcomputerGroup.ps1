param([string]$group = "group", [string]$file = "file", [string]$user = "user", [string]$password = "password")
#
...
# Using the endpoint names from the cmdline filename, substitute them into multiple copies of the following 'stanza', leaving result in el
...
#
	...
	$b3 = "bay.edu"
        ...
        $url = $baseurl + '/login'
        $r1 = Invoke-WebRequest -Uri $url -Method GET -Headers $headers
        ...
        #     comes back as a string.
            $r2 = Invoke-WebRequest -Uri $url -Method POST -Headers $headers -Body $newXml 
